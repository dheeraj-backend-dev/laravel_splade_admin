...
class StatesController extends Controller
{
    ...
    public function store(Request $request, CreateStateForm $form)
    {
        ...
        State::create($data);
        Splade::toast("State Created successfully")->autoDismiss(5);
    ...
    public function update(Request $request, State $state, UpdateStateForm $form)
    {
        $data = $form->validate($request);
        $state->update($data);
        Splade::toast("State Updated successfully")->autoDismiss(5);
        return to_route('admin.states.index');
    }
    ...
    {
        $state->delete();
        Splade::toast("State successfully deleted.")->autoDismiss(5);
        return back();
    }
}
